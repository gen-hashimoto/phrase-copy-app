...
**ゲスト**: 画面設計上、未ログイン時のデータはフロントエンドのuseState保持想定（
...
無料ログインユーザーは50件まで、プレミアムユーザーは無制限とする。
...
- **概要**: CSVから一括登録（Append / Overwrite）。ログインモード向けの未実装機能
- **Auth**: 必須
- **Body（例）**: `mode`: `append` \| `overwrite`、`file` または `csv`: CSVデータ
...
- `reorder` / `import` のトランザクション・上限・バリデーション（
  空行・最大件数）
- フレーズ登録上限（無料50件、プレミアム無制限）の判定をAPI層でどう表現するか
- CSVインポート / エクスポート、ゲスト向けテキストインポート / エクスポートの最終仕様
